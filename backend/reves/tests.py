# ---------------------------------------------------------------------------
# TESTS INSCRIPTION UTILISATEURS (unicité et charge simple)
# ---------------------------------------------------------------------------
...
class UserRegistrationTest(TestCase):
    def test_username_uniqueness(self):
        """On ne doit pas pouvoir créer deux utilisateurs avec le même nom."""
        User.objects.create_user(username="dupuser", password="azerty123")
        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                User.objects.create_user(username="dupuser", password="autrepass")

    def test_20_users_can_register_and_login(self):
        """Le site doit permettre à 20 utilisateurs de s'inscrire et se connecter sans erreur."""
        usernames = [f"user{i}" for i in range(20)]
        passwords = [f"pass{i}ABC" for i in range(20)]
        clients = [Client() for _ in range(20)]
        # Création des utilisateurs
        for u, p in zip(usernames, passwords):
            User.objects.create_user(username=u, password=p)
        # Connexion simultanée (threadée pour simuler la concurrence)
        results = [None] * 20
        def login_user(idx):
            logged = clients[idx].login(username=usernames[idx], password=passwords[idx])
            results[idx] = logged
        threads = [threading.Thread(target=login_user, args=(i,)) for i in range(20)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
        self.assertTrue(all(results), "Tous les utilisateurs doivent pouvoir se connecter en même temps.")
import datetime
import io
import struct
import wave
...
class UploadAbuseProtectionTest(TestCase):
    ...
    def test_eighth_upload_of_the_day_is_refused(self):
        ...
        with patch('reves.views.start_transcription_async', return_value=True):
...
class SecureDreamAudioAccessTest(TestCase):
    def setUp(self):
        cache.clear()
        self.client = Client()

        self.owner_user, self.owner_profil = make_user_with_profil(username='audio-owner')
        self.other_user, _ = make_user_with_profil(username='audio-other')
    ...
    def test_owner_can_access_audio(self):
    ...
    def test_other_user_gets_403(self):
        self.client.login(username='audio-other', password='testpass123')
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 403)
    ...
    def test_direct_media_audio_url_is_forbidden(self):
        response = self.client.get('/media/reves_audio/guess.wav')
        self.assertEqual(response.status_code, 403)